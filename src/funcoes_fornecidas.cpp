...
void scan_quote_string(char *str) {
    ...
    // Ignorar espaços iniciais ou nova linha
    while ((R = getchar()) != EOF && isspace(R));

    if (R == '\"') {
        // Ler até o fechamento das aspas
        if (scanf("%[^\"]", str) != 1) {
            strcpy(str, ""); // Se falhar, retorna string vazia
        }
        getchar(); // Ignorar aspas finais
    } else {
        // Caso não haja aspas, ler como string comum
        ungetc(R, stdin); // Recolocar o caractere lido
        scanf("%s", str);
    }
}
